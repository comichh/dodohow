...
$(function() {
  ...
  */
  var leftStart=parseInt((getBrowserWidth()-1280)/2);
  $('#DeepSearch').css('left',leftStart+"px");
  $('#DeepSearch').css('top',"250px");
});
...
function searchActivityBycodeAddress(addressFieldName) {
	...
	geocoder.geocode( {'address': address}, function(results, status) {
		if (status == google.maps.GeocoderStatus.OK) {
			...
			var distance=50;
			var url='FactivitylistNearByCenter.php'+
				"?lat="+lat+
				"&lng="+lng+
				"&distance="+distance
			$('#factivitylist').attr('src',url);
...
function searchActivityByFilter(){
	...
	var getUrl="factivitylistAdvance.php?SearchMode=3&type="
