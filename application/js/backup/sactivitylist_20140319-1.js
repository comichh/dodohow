...
function refreshMyFavSport(){
	$.ajax({
			...
			success: function(Jdata) {
				...
				var dataLength=objectData.length;
				for(var index=0;index<dataLength;index++){
					//contentStr+="<li style='width:250px'><a tabindex='-1' href='#'>"+objectData[index].TrainName+"<span class='append' style='position:relative;float:right;top:-2px'><img src='./images/minus.png' width='24' /></span></a></li>"
					contentStr+="<li style='width:250px'><a tabindex='-1' href='#'>"+objectData[index].TrainName+"</span><span class='append' style='position:relative;float:right;top:-2px'><img src='./images/minus.png' width='24' onclick='deleteMyFavSport("+objectData[index].UserID+","+objectData[index].TrainID+")'/></span></a></li>";
				}
				window.parent.updateFavSportPlanDetail(contentStr);			
			},
			error: function() {
				return eval('[{}]');
				alert("ERROR!!! by getDataByJSON()");
