...
function getActivesFromBound(){
	...
	for(var i=0;i<markers.length;i++){
		...
		if(((markers[i].getPosition().lat()>=SWLatlng.lat())&&(markers[i].getPosition().lat()<=NELatlng.lat()))
			...
			str+='<div style="padding:2px;"><div style="padding:5px;background:#ffffff;border:1px solid #CD661D;border-radius: 0.5em 0.5em 0.5em 0.5em;"><span><img src="./images/play.jpg" width=72 onclick="setMarkerCenetr('+i+')" style="cursor:pointer" /></span><span style="position:relative;top:-20px;left:10px"><span id="choose" class="label label-info" onclick="getHistoryByAjax('+jsonTripArray[i].id+',\''+jsonTripArray[i].deviceID+'\')" style="cursor:pointer">Detail</span>&nbsp<span style="cursor:pointer">'+shareName+'</span></span></div></div>';
		}
	}
	
	if(str==''){
		$('#activitys').html('<div align="center" style="position:relative;top:20px;"><img src="./images/blueloading.gif" />');
	}else{
