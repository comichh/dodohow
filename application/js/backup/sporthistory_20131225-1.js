	 ...
	 $('#container').highcharts({
		...
		*/
		$('#container').highcharts({
            ...
            subtitle: {
                text: '<p>Source: sport4u sport for you</p><p>Trend analyze for you</p>'
            },
            yAxis: [{ // Primary yAxis
                labels: {
                    format: '{value} bits',
			...
			if(jsonArray.length>0){
				...
				if(jsonArray[0].Detail!=''){
					...
					*/
					...
					drawData(heartbit,speed,height);
			...
			if(jsonArray.length>0){
				...
				for(var index=0;index<lengthval;index++){
					str+='<li style="border-top: 1px solid #ccc;"><a href="#" onclick="getHistory(\'./common/lib/php/getdata.php\','+jsonArray[index].id+')">'
						+'<h6>'+jsonArray[index].Start_time+'<br>'+jsonArray[index].train_name+'</h6></a></li>';
				}
...
function drawData(dataArray1,dataArray2,dataArray3){
