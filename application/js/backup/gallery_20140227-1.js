...
function showImage(ImgName,imageUrl,index) {
	...
	pictureIndex=index;
	var lat=parseFloat(jsonArray[index].pic_lat);
	var lng=parseFloat(jsonArray[index].pic_lng);
	if((lat!=999)&&(lng!=999)){
		var latlng=new google.maps.LatLng(lat,lng);
		map.setCenter(latlng);
		picMarker.setPosition(latlng);
		window.parent.setMapCenterByImage(lat,lng,imageUrl);
		map.setCenter(latlng);
	}
	/*
...
function initGallery(){
	...
	setTimeout(function(){
		google.maps.event.trigger(map, "resize");
		setStartPositionOnMap();
	},1000);
}

function updatePictureInfo(index,tripID,picID){
	var title=$('#Title_'+index).val();
	var description=$('#description_'+index).val();
	var latLng=$('#position_'+index).html();
	var latLngArray=latLng.split(",");
	$.ajax({
		url:'./common/lib/php/UpdatePictureInfo.php',
		type:'POST',
		data:'picTrip='+tripID
			+'&picId='+picID
			+'&picTitle='+encodeURIComponent($('#Title_'+index).val())
			+'&picDescription='+encodeURIComponent($('#description_'+index).val())
			+'&picLat='+latLngArray[0]
			+'&picLng='+latLngArray[1],
...
var picindex;
function setParentGalleryMap(index,lat,lng){
	picindex=index;
 	window.parent.myModalMapShow(lat,lng);
}

function updateLatlng(lat,lng){
	$('#position_'+picindex).html(lat+","+lng);
}
