...
$(function() {	
	...
	$('#accordion h3.ui-accordion-header').click(function(){
		 ...
		 switch(tabName){
			case "ShowMap":
				...
				break;
			case "ShowTrendChart":
				if($('#ChartAreaContent').css('display')=="none"){
					if(height.length>0){
						drawData($('#AxisGroup .active').attr('value'));
					}
				}
				break;
			case "TripPicture":
				if(typeof(activityID)!=="undefined"){
					if($('#SportGallery').attr('src')!=''){
						setTimeout(function(){
							google.maps.event.trigger(map, "resize");
							document.getElementById("SportGallery").contentWindow.setStartPositionOnMap();
						},500);
					}
				}
				break;
	...
	$("#UpdateGalleryGisInfo").click(function(){
		$("#myModalMap").hide();
	});

	$('#DistanceAxis').click(function(){
		if(height.length>0){
			drawData("1");
		}
	});
...
function initialize(lat,lng) {
  ...
  var mapOptionsGallery = {
	zoom: 14,
	center: new google.maps.LatLng(lat, lng),
	mapTypeId:google.maps.MapTypeId.ROADMAP
  };
  mapGallery = new google.maps.Map(document.getElementById('MapGallery'),mapOptionsGallery);
  ...
  markerGallery = new google.maps.Marker({
	map:mapGallery,
  ...
  google.maps.event.addListener(markerGallery, 'dragend', function(event){ 
	var latlng=markerGallery.getPosition();
	document.getElementById("SportGallery").contentWindow.updateLatlng(event.latLng.lat(),event.latLng.lng());
  });
				...
				var flightPlanCoordinates=[];
			 	if(jsonArray[0].Detail!=''){
					...
					for(var index=0;index<detailObj.length;index++){
						...
						if(index==0){
							sportDuration.push(0);
						}else{
							runtimeSec+=getDatetimeValue(detailObj[index-1].start_time,detailObj[index].start_time);
							sportDuration.push(runtimeSec);
						}
					}
...
function getDatetimeValue(datetime1,datetime2){

	return (splitDatetime(datetime2).getTime()-splitDatetime(datetime1).getTime())/1000;
...
function splitDatetime(datetime){
	...
	return new Date(parseInt(dateArray[0]), dateArray[1], dateArray[2], timeArray[0], timeArray[1], timeArray[2])
}
...
function drawData(chartType){
	...
	switch(chartType){
		case "1":
			chart.options.xAxis[0].labels={
				format: '{value} km'
			};
			chart.options.xAxis[0].categories=eachDistance;
			tickValue=parseInt(eachDistance.length/rangeLevel);
			
			break;
		case "2":
			chart.options.xAxis[0].labels={
				formatter: function() {
					return calHrAndMinWithSecond(this.value);
				}
			};
			chart.options.xAxis[0].categories=sportDuration;
			tickValue=parseInt(sportDuration.length/rangeLevel);
			break;
	}
	chart.options.xAxis[0].tickInterval=tickValue;
	var chareWidth=parseInt($('#accordion').css('width').replace("px",""));
	chart.options.chart.width=chareWidth;
	chart = new Highcharts.Chart(chart.options);
	chart.render();
    chart.series[0].setData(heartbit,false);
	chart.series[1].setData(speed,false);
	chart.series[2].setData(height,false);
	chart.redraw();
}
...
function myModalMapShow(lat,lng){
	$('#myModalMap').show();
	google.maps.event.trigger(mapGallery,"resize");
	if((parseFloat(lat)==999)||(parseFloat(lng)==999)){
		var lat=parseFloat(detailObj[0].latitude);
		var lng=parseFloat(detailObj[0].longitude);
		var latlng=new google.maps.LatLng(lat,lng);
		markerGallery.setPosition(latlng);
		mapGallery.setCenter(latlng);
	}else{
		var latlng=new google.maps.LatLng(parseFloat(lat),parseFloat(lng));
		markerGallery.setPosition(new google.maps.LatLng(lat,lng));
		mapGallery.setCenter(latlng);
	}
}


function drawLapPathLine(lapCoordinates){
	var flightPlanCoordinates=[];
	var eachPoint;
	var bounds = new google.maps.LatLngBounds();
	for(var i=0;i<lapCoordinates.length;i++){
		eachPoint=lapCoordinates[i].split(",");
		flightPlanCoordinates.push(new google.maps.LatLng(parseFloat(eachPoint[0]),parseFloat(eachPoint[1])));
		bounds.extend(new google.maps.LatLng(parseFloat(eachPoint[0]),parseFloat(eachPoint[1])));
	}
	if(flightLapPath){
		flightLapPath.setPath(flightPlanCoordinates);
		flightLapPath.setVisible(true);
	}else{
		flightLapPath = new google.maps.Polyline({
			path:flightPlanCoordinates,
			strokeColor: '#1C86EE',
			strokeOpacity: 1.0,
			strokeWeight: 5,
