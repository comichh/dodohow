...
function loadGPXFileIntoGoogleMap(filename) {
	$.ajax({url: filename,
		...
		success: function(data) {
			...
			chart.redraw();
			dataType=2;
			reDrawKmStop(elevations.length);
		}
	});
}
...
function reDrawKmStop(SampleCount){
	...
	var totalCalMiles=id*1000*SampleCount;
	var pointIndex=parseInt(totalCalMiles/totalMiles);
		
	if(totalCalMiles%totalMiles>0){
		var round=parseInt(SampleCount/(totalCalMiles/totalMiles));
	}else{
		var round=parseInt(SampleCount/(totalCalMiles/totalMiles))+1;
	}
			
	for(var indexKm=1;indexKm<=round;indexKm++){
		var markerKM = new MarkerWithLabel({
		   position: new google.maps.LatLng(parseFloat(elevations[pointIndex*indexKm].location.lat()),parseFloat(elevations[pointIndex*indexKm].location.lng())),
		   map:map,
		   visible:true,
		   draggable: false,
		   raiseOnDrag: true,
		   labelContent: "<div align='center' style='border:1px solid #ffffff;background:#008B00;'><div align='center' style='padding:2px;border:1px solid #ffffff;background:#008B00;width:40px;height:20px'><div style='position:relative;color:#ffffff;width:40px;top:-5px;weight:900;'>"+indexKm*id+" km</div></div></div>",
...
function getActivityRoutePathKM(){
	...
	//最後一點補上最後一段距離的值
	distanceArray.push(Math.floor(distanceadd*10)/10);
	totalMiles=(parseInt(distanceadd)+1)*1000;
...
function redrawRoutePath(){
	...
	outSourceMarker[0].setVisible(false);
	outSourceMarker[1].setVisible(false);
	chart = $('#container').highcharts();
