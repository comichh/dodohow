			...
			if(DetailTrip.length>0){
				for(var index=0;index<DetailTrip.length;index++){
					if((DetailTrip[index].pic_lat!=999)&&(DetailTrip[index].pic_lng!=999)){
						var eachPhotomarker = new google.maps.Marker({
						...
						google.maps.event.addListener(eachPhotomarker, 'click', (function(eachPhotomarker,lat,lng,descript,pic_name) {            
							return function() {
								var detailWindow = new google.maps.InfoWindow({
									  content: '<div><img src="'+pic_name+'" style="max-width:300px;height:180px" /></div>',
								});
								map2.setCenter(eachPhotomarker.getPosition());
								setTimeout(function(){
									detailWindow.open(map2,eachPhotomarker); 
								},500);	       
							}
						})(eachPhotomarker, DetailTrip[index].pic_lat, DetailTrip[index].pic_lng,DetailTrip[index].pic_descript,DetailTrip[index].pic_name));
						photoMarker.push(eachPhotomarker);
					}
				}
