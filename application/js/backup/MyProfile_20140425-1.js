...
function getUserSpaceInfo(){
	...
	$.ajax({
		...
		success: function(Jdata) {
			...
			seriesData.push({name:uploadedSpace,y:usedSpacePercentage,color:"#9BCD9B"});
			seriesData.push({name:FreeSpace,y:freeSpacePercentage,color:"#1C86EE"});
			chart = $('#StorageChart').highcharts();
			chart.options.title.text=storageStatu;
			chart.render();
...
function changeLanguage(){
	var langID=$('#UseLanguage').val();
	$.ajax({
		url:'./common/lib/php/ChangeLanguage.php',
		type:'POST',
		data:'LangID='+encodeURIComponent(langID),
		success: function(response) {
			if(response==="1"){
