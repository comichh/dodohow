...
function reduce($point,$point1){
global $total_point,$max_point,$chenge_unit;
...
//  if (count($need) <$max_point-1   ){
       $check_point=dis($key1,$key,$point,$point1);   
...
function dis($p1,$p2,$point,$point1)              // p1 , p2 只是  key ,
{
global $total_point,$max_point,$chenge_unit;
// $point[1]    // 第一點緯度
// $point1[1]    // 第一點經度
global $R,$pi,$lim_d,$lim_angle,$d;
  ...
  $sin_1_Longitude=sin($point1[$p1]*$chenge_unit);    // sin （第一點經度）
  $cos_1_Longitude=cos($point1[$p1]*$chenge_unit);    // cos （第一點經度）
  $sin_2_Longitude=sin($point1[$p2]*$chenge_unit);    // sin （第二點經度）
  $cos_2_Longitude=cos($point1[$p2]*$chenge_unit);    // cos  (第二點經度） 

  $delta_Longitude=$point1[$p2]*$chenge_unit-$point1[$p1]*$chenge_unit;  
  $delta_Latitude= $point[$p2]*$chenge_unit-$point[$p1]*$chenge_unit;    
  // 距離
