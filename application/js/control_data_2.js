...
$(function(){
  $("#list").jqGrid({
		datatype: "local",                            // sorttype: 只有在這種模式下才能用
    editurl:"ajax/control_del.php",               // 刪除用的，navGrid都是往這邊
	    ...
	    {name:'trainid',index:'trainid',width:5,align:"center",search:false,hidden:true,key:true},
			...
			{name:'PublishTime',index:'PublishTime', width:55, align:"center",sorttype:"date",search:false,hidden:PublishTime_hide},
			{name:'StartTime',index:'StartTime', width:55, align:"center",sorttype:"date",search:false},
      {name:'public_access',index:'public_access', width:17, align:"center",search:false,hidden:Public_access_hide},      
			{name:'slope_pic',index:'slope_pic', width:60, align:"center",search:false,sorttype:'float'},
      ...
      {name:'device_name',index:'device_name', width:40,align:"center",search:false},
			...
			{name:'PraiseCount',index:'PraiseCount', width:22,align:"center",search:false,sorttype:'integer'},
			{name:'CollectCount',index:'CollectCount', width:22,align:"center",search:false,sorttype:'integer'},
    ...
    sortname: 'id',
    sortorder: "desc", 
    multiselect: multiselect,              //  這個下完就會出現前面框框
		rowList : [5,10,20,30,50,100],
		//caption: titleListOfActivitiesByFilter,   // 不知道是什麼 ，先取消，但是我有帶一個語系過來           
		loadComplete: function () {
			$(this).find(">tbody>tr.jqgrow:visible:odd").addClass("myAltRowClass");
		},
    //jqGridAddEditAfterComplete:function(data){
    // alert (data);
    //}
...
function getDataFromDistance(){
	...
	setTimeout(function(){
		var fullUrl="./common/lib/php/GetActivityByDistanceAndCenter.php?"+
		"&centerlat="+lat+
		"&centerlng="+lng;
		$.ajax({
			url:fullUrl,
          ...
          var public_access='';
            ...
            if ( objectData[index].public_access ==1){
              public_access='Yes';
            }else{public_access='No';}
            contentStr+="public_access:'"+public_access+"',";

						contentStr+="displayname:'<img src=\""+objectData[index].UserImage+"\" width=\"36\" alt=\""+objectData[index].displayname+"\" /><p>"+objectData[index].displayname+"</p>',";
...
//							contentStr+="Photo:'<div align=\"center\"><img src=\"./images/photocollect.png\" width=\"28\" /><br/>("+objectData[index].PictureCount+")</div>',";
        		contentStr+="Photo:'"+objectData[index].PictureCount+"',";
						}else{
							contentStr+="Photo:'',";
						}
...
/*
function slope_account(number,http_path)
{
  switch (number)
  {
  case '0.5':
     slope_pic="<img  src='http://"+http_path+"/pic/train_detail/helf_star.png'></img>";
  break;
  case '1':
      slope_pic="<img  src='http://"+http_path+"/pic/train_detail/all_star.png'></img>";
    break;
  case '1.5':
      slope_pic="<img  src='http://"+http_path+"/pic/train_detail/all_star.png'></img>";
      slope_pic+="<img  src='http://"+http_path+"/pic/train_detail/helf_star.png'></img>";
