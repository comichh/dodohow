    ...
    var societies=$('#ShareGroupList').val();   
		...
		$.ajax({
			...
			data:'TrainID='+activityID+'&SportName='+encodeURIComponent($('#ActivityName').val())
					...
					+'&elat='+detailObj[lastIndex].latitude+'&elng='+detailObj[lastIndex].longitude+'&desc='+$('#Description').val()+"&sporttype="+$('#SelectSportType').val(),
		...
		success: function(Jdata) {
			...
			if(jsonArray.length>0){
				...
				for(var index=0;index<lengthval;index++){
					...
					str+='<li style="border-top:1px solid #ccc;cursor:pointer" id="list_'+index+'" onclick="getHistory(\'./common/lib/php/getdata.php\','+jsonArray[index].id+','+index+')">'
						+'<table style="width:260px;height:60px" cellapdding=0 cellspacing=0><tr><td><img id="img_"'+index+'" src="./images/activity.png" onclick="openDialog('+jsonArray[index].id+','+index+','+shareRoute+','+shareMyRecord+','+sharePhoto+')"></td><td style="font-size:12px;font-family: verdana;font-weight:bold;" width="80%"><div id="Activity_'+jsonArray[index].id+'_Time" style="padding:1px">'+jsonArray[index].Start_time+'</div><div id="Activity_'+jsonArray[index].id+'">'+train_names+'</div><div id="shareImage_'+jsonArray[index].id+'" style="padding:1px">'+shareImage+'</div></td></tr></table></li>';
				}
				str+='</ul>';
				str+='</div>';
