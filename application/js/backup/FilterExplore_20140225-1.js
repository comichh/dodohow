...
function searchActivityBycodeAddress() {
	...
	geocoder.geocode( { 'address': address}, function(results, status) {
		if (status == google.maps.GeocoderStatus.OK) {
			...
			document.getElementById("factivitylist").contentWindow.getDataFromDistance(typelat,typelng,rangeSearchDistance,minSportDistance,maxSportDistance,UserSearch,SportNameSearch,SportDescriptionSearch,SportDurtionSearch);
		} else {
			var inputStr=$('#AddressSearch').val();
			if(inputStr.trim()==""){
				document.getElementById("factivitylist").contentWindow.getDataFromDistance(999,999,rangeSearchDistance,minSportDistance,maxSportDistance,UserSearch,SportNameSearch,SportDescriptionSearch,SportDurtionSearch);
			}else{
