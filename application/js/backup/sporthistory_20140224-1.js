				...
				var flightPlanCoordinates=[];
			 	if(jsonArray[0].Detail!=''){
					...
					if(parseInt(jsonArray[0].PhotoCount)>0){
						$('#SportGallery').attr('src','gallery.php?tripID='+getID);
					}else{
...
function getenc(){
	...
	var divValue=parseInt(detailCount/600);
	var pointIndex=0;
	var modValue=0;
	...
	for(var index=0;index<detailObj.length;index++){
		...
		if(modValue==0){
			points[pointIndex]=[parseFloat(detailObj[index].latitude), parseFloat(detailObj[index].longitude)];
			pointIndex+=1;
		}
	}
