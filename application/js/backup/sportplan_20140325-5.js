...
$(function () {
	...
	$('#container').highcharts({
		...
		tooltip: {
			...
			formatter: function() {
				return '<font size="1">'+titleSportPlanMiles+':'+this.x+' '+titleSportPlanKilometer+'</font><br><font size="1">'+titleSportPlanElevation+':'+parseInt(this.y)+' '+titleSportPlanMeter+'</font>';
			}
		},
		legend: {
			layout: 'vertical',
					...
					point: {
						events: {
							...
							select: function() {
								...
								//alert(routeResult[this.x].location.lat());
								/*
...
function loadGPXFileIntoGoogleMap(filename) {
	$.ajax({url: filename,
		...
		success: function(data) {
			...
			if(resultlen>0) moveStepmarker.setVisible(true);
