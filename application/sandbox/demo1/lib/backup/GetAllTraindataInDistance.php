<?php
	...
	try {
		...
		if($num_rows>0){
			while($row = mysql_fetch_array($result)){
				...
				$strdata.="PraiseCount:".gePraiseCount($row['id']).",";
				$strdata.="FavSportList:".getFavListCount($row['id']);
				$strdata.="},";
			}
	...
	function getFavListCount($id){
		try{
			$sqlstr.=" SELECT count(*) as TotalCount FROM tbl_planning where tain_planning=".$id;
			mysql_query("set names 'utf8'"); 
			$result = mysql_query($sqlstr) or die($sqlstr);
	...
	function gePraiseCount($id){
		try{
			$sqlstr.=" SELECT count(*) as TotalCount FROM tbl_praise where 	praise_train_key=".$id;
			mysql_query("set names 'utf8'"); 
			$result = mysql_query($sqlstr) or die($sqlstr);
			$row = mysql_fetch_array($result);
			return $row['TotalCount'];
