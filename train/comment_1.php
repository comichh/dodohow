...
$show_white="<img src='$comment_pic_path/show_-.png'/>";
...
// 最下面的回應框
   $comment_form="
     ...
     <TextArea style='width:100%;' Cols='2' Rows='3' Wrap='Virtual' class='input-block-level_1' placeholder='回覆內容' name='content'></TextArea>               
...
while ($row=mysql_fetch_array($result_1))
{
  ...
  $top_content_m="<div id='top_content_m'>$row[train_name] </div>";
  $creator_img="<img width='100px' src='$work_web/upload/user/$row[image]'></img>";
}


$train_top_head="<html><body><div id='top_content'>";
$train_top_head.="
<div id='top_pic_div'>
<div id='top_pic_l'></div>
<div id='top_pic_m'>$top_content_m</div>
<div id='top_pic_r'></div>
</div><br>
";
$train_top_head.="<div id='top_content_1'><span class='te'>$creator_img</span><span class='te'>".$train_description."</span><hr>";
    ...
    { 
      ...
      $comment_origin_form.="<tr><td class='triangle' >".$img."</td><td  class='triangle'>".$triangle."</td><td>";
...
// 這邊撈一次黑名單,  黑名單有兩種，若是訓練紀錄擁有者，擋全部留言。巢狀裡面也可以個別擋。
    ...
    // 主畫面的呈現
    echo $train_top_head;
    echo "<div style='color:white'> <a id='show_total_comment' href='#'>$show_white </a>$lang[show_comment_1] :$total_0_result_number</div>";
     if ($black_user !=1){   
...
switch ($row['indent'])
      ...
      $comment_origin_form.="$img<div class='name'>".$row['displayname'].$reply_men."</div>";       
      $comment_origin_form.="<div class='input-block-level'>".$row['content']."</div>";
        
      $comment_origin_form.="<div id='content_new_reply_".$row['id']."_".$train_id."_".$row['id_comment']."'><TextArea style='width:100%' Cols='40' Rows='3' Wrap='Virtual' class='input-block-level' placeholder='回覆內容' name='content'></TextArea>";
      $comment_origin_form.="<button id='submit_new_reply_".$row['id']."_".$train_id."_".$row['id_comment']."'>$lang[submit]</button></div>";       
