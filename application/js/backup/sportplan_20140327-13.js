...
function loadGPXFileIntoGoogleMap(filename) {
	$.ajax({url: filename,
		...
		success: function(data) {
			...
			var elevationVal=0.0;
			for(var i=0;i<resultlen;i++){
				elevationVal=routeResult[i].location.elevation();
				if(maxElevationValue<elevationVal){
					maxElevationValue=elevationVal;
				}
				elevationArray.push(maxElevationValue);
			}
...
function addMarker(latlng, doQuery) {
	if((getDataType===0)||(getDataType===1)){
		var marker = new google.maps.Marker({
			position: latlng,
			map:map,
			icon:'./images/Active_Marker.png',
