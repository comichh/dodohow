...
function initialize() {
  ...
  google.maps.event.addListener(map, 'click', function(event){ 
		...
		$('#position_'+pictureIndex).val(event.latLng.lat()+","+event.latLng.lng());
  });
		...
		success: function(Jdata) {
			...
			if(jsonArray.length>0){
				for(var index=0;index<jsonArray.length;index++){
					divstr+='<li>';
					divstr+='	<a class="thumb" name="leaf" href="'+jsonArray[index].ImageName+'" title="Title #0">';
					divstr+='		<img id="p'+jsonArray[index].index+'" src="'+jsonArray[index].ImageName+'" style="width:75px;height:75px" alt="Title #0" onclick="showImage(\'p'+jsonArray[index].index+'\',\''+jsonArray[index].ImageName+'\','+index+')" />';
					divstr+='	</a>';
					divstr+='	<div class="caption">';
					divstr+='		<div class="image-title">名稱'+'<input type="text" id="Title_'+index+'" size=20 value="'+jsonArray[index].ImageTitle+'" /></div>';
					divstr+='		<div class="image-desc">位置'+'<input type="text" id="position_'+index+'" size=40 value="'+jsonArray[index].ImageLatlng+'"/></div>';
					divstr+='		<div class="image-desc">簡述'+'<input type="text" id="description_'+index+'" size=60 value="'+jsonArray[index].ImageDescription+'"/></div>';
					divstr+='		<div class="download">';
					divstr+='			<a href="'+jsonArray[index].ImageName+'" target="_blank">Download Original</a><button onclick="updatePictureInfo('+index+')">更新</button>';
					divstr+='		</div>';
					divstr+='	</div>';
					divstr+='</li>';
				}
...
function setStartPositionOnMap(lat,lng){
