...
$(function() {	
	...
	$('#UpdateSettings').click(function(){
		var statu;
		if ($("#ShareMyRouteToGroup").prop('checked')){
			statu=1;
		}else{
			statu=0;
		}
		...
		$.ajax({
			...
			data:'TrainID='+activityID+'&SportName='+encodeURIComponent($('#ActivityName').val())
					+'&statu='+statu+'&slat='+detailObj[0].latitude+'&slng='+detailObj[0].longitude
					+'&elat='+detailObj[lastIndex].latitude+'&elng='+detailObj[lastIndex].longitude,
		...
		success: function(Jdata) {
			...
			if(jsonArray.length>0){
				...
				for(var index=0;index<lengthval;index++){
					...
					str+='<li style="border-top: 1px solid #ccc;"><a href="#" onclick="getHistory(\'./common/lib/php/getdata.php\','+jsonArray[index].id+','+index+')">'
						+'<table><tr><td><img src="./images/activity.png" onclick="openDialog('+jsonArray[index].id+','+index+','+shareRoute+','+shareMyRecord+','+sharePhoto+')"></td><td><h6><span id="Activity_'+jsonArray[index].id+'_Time">'+jsonArray[index].Start_time+'</span><span id="shareImage_'+jsonArray[index].id+'">'+shareImage+'</span><br><span id="Activity_'+jsonArray[index].id+'">'+train_names+'</span></h6></td></tr></table></a></li>';
				}
				str+='</ul>';
				str+='</div>';
