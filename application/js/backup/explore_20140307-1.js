...
$(function() {
  ...
  $('#MyProfile').click(function(){
	  //document.getElementById("Gridframe").contentWindow.clearGridData();
  });
  ...
  */
	$('#myModal').on('hidden.bs.modal', function () {
       if(photoMarker.length>0){
			for(var i=0;i<photoMarker.length;i++){
				photoMarker[i].setMap(null);
			}
			photoMarker=[];
		}
	})
...
function initialize() {
	...
	//-------------------- add control on detail map --------------------------------//
	var photoControlDiv = document.createElement('div');
	var photoControl = new addPhotoControlOnMap(photoControlDiv, map2);

	photoControl.index = 1;
	map2.controls[google.maps.ControlPosition.TOP_LEFT].push(photoControlDiv);
	//-------------------------------------------------------------------------------//
   ...
   activityWindow2 = new google.maps.InfoWindow({
      content: ""
   });
  ...
  google.maps.event.addListener(activityWindow, 'closeclick', function(){
   ...
   google.maps.event.addListener(activityWindow2, 'closeclick', function(){
	for (i=0; i<line.length; i++){                           
		line[i].setMap(null); 
	}
  });
...
function addPhotoControlOnMap(controlDiv, map) {
  ...
  controlUI.title = 'Click to Open Photo Gallery';
  ...
  controlText.innerHTML = '<img src="./images/picture.png"/>Photo';
  controlUI.appendChild(controlText);

  google.maps.event.addDomListener(controlUI, 'click', function() {
    if(photoMarker.length==0){
		getDetailTripPicture(tripID,true);
	}else{
		if(photoMarker[0].getVisible()){
			for(var i=0;i<photoMarker.length;i++){
				photoMarker[i].setVisible(false);
			}
		}else{
			for(var i=0;i<photoMarker.length;i++){
				photoMarker[i].setVisible(true);
			}
		}
	}
  });
...
var photoMarker=[];
function getDetailTripPicture(ID,open){
	var fullUrl="./common/lib/php/GetGalleryPicture.php?tripID="+ID;
	$.ajax({
		url:fullUrl,
		type:"GET",
		contentType:"application/json;charset=utf-8",
		success: function(Jdata) {
			DetailTrip=eval('('+Jdata+')');
			var divstr='';
			if(DetailTrip.length>0){
				for(var index=0;index<DetailTrip.length;index++){
					if((DetailTrip[index].pic_lat!=999)||(DetailTrip[index].pic_lng!=999)){
						var marker = new google.maps.Marker({
							  position: new google.maps.LatLng(parseFloat(DetailTrip[index].pic_lat),parseFloat(DetailTrip[index].pic_lng)),
							  map:map2,
							  title:DetailTrip[index].pic_descript,
							  icon:'./images/picture.png'
						});
					}
					google.maps.event.addListener(marker, 'click', (function(marker,lat,lng,descript,pic_name) {            
						 return function() {  
							map2.setCenter(marker.getPosition());
							detailWindow.setContent('<div><img src="'+pic_name+'" style="max-width:300px;height:180px" /></div>');
							detailWindow.open(map2,marker);          
						 }
					})(marker, DetailTrip[index].pic_lat, DetailTrip[index].pic_lng,DetailTrip[index].pic_descript,DetailTrip[index].pic_name));
					photoMarker.push(marker);					
					/*
					divstr+='<li>';
					divstr+='	<a class="thumb" name="leaf" href="'+jsonArray[index].pic_name+'" title="Title #0">';
					divstr+='		<img id="p'+jsonArray[index].pic_id+'" src="'+jsonArray[index].pic_name+'" style="width:75px;height:75px" alt="Title #0" onclick="showImage(\'p'+jsonArray[index].pic_id+'\',\''+jsonArray[index].pic_name+'\','+index+')" />';
					divstr+='	</a>';
					divstr+='	<div class="caption">';
					divstr+='		<div><img src="./images/map.png" id="ShowMap" onclick="setParentGalleryMap('+index+','+jsonArray[index].pic_lat+','+jsonArray[index].pic_lng+')" /></div>';
					...
					divstr+='			<a href="'+jsonArray[index].pic_name+'" target="_blank">Download Original</a><button onclick="updatePictureInfo('+index+','+jsonArray[index].pic_train_key+','+jsonArray[index].pic_id+')">更新</button>';
					divstr+='		</div>';
					divstr+='	</div>';
					divstr+='</li>';
					*/
		...
		error: function() {
			alert('function::getDetailTripPicture() error!');
		}
	});
}
				...
				for(var i=0;i<totalcount;i++){             
					...
					google.maps.event.addListener(marker, 'click', (function(marker, tripID, deviceID,imageName,trainName,fullname,lTotalDistance,lTotalTime) {            
						 return function() {  
							...
							activityWindow.setContent('<div style=\"width:200px;height:120px\"><div style=\"width:98%;background:#cfcfcf\"><img src="../upload/user/'+imageName+'" style="border:1px #cecece solid;width:72px;height:54px">'+trainName+'</div><div style=\"font-size:10px;font-family:verdana;font-weight:900;\">'+parseFloat(lTotalDistance/1000)+"公里</font>&nbsp;&nbsp;&nbsp;&nbsp;<font style=\"font-size:10px;font-family:verdana;font-weight:900;\">"+calHrAndMin(lTotalTime)+'</font></div><div><span class="badge badge-success" style="cursor:pointer;" onclick="getHistoryByAjax('+tripID+',\''+deviceID+'\')">詳細資料</div></div>');
...
function locateMapRoutePathAndPosition(lat,lng,tripID,deviceID,trainName,imageName,lTotalDistance,lTotalTime){
	...
	activityWindow2.setContent('<div style=\"width:200px;height:120px\"><div style=\"width:98%;background:#cfcfcf\"><img src="../upload/user/'+imageName+'" style="border:1px #cecece solid;width:72px;height:54px">'+trainName+'</div><div style=\"font-size:10px;font-family:verdana;font-weight:900;\">'+lTotalDistance+"公里</font>&nbsp;&nbsp;&nbsp;&nbsp;<font style=\"font-size:10px;font-family:verdana;font-weight:900;\">"+lTotalTime+'</font></div><div><span class="badge badge-success" style="cursor:pointer;" onclick="getHistoryByAjax('+tripID+',\''+deviceID+'\')">詳細資料</div></div>');
...
function updateFavSportPlanDetail(content){
	$('#MyFavSport').html(content);	
}
...
function codeAddress() {
  ...
  geocoder.geocode( { 'address': address}, function(results, status) {
    if (status == google.maps.GeocoderStatus.OK) {
        ...
        document.getElementById("factivitylist").contentWindow.getDataFromDistance(typelat,typelng,$('#DistanceFromCenter').val());
    } else {
      alert('Geocode was not successful for the following reason: ' + status);
    }
  });
