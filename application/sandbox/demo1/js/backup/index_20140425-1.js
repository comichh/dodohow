		...
		success: function(Jdata) {
			...
			if(jsonArray.length>0){
				...
				if(jsonArray[0].Detail!=''){
					...
					$('#ReportTitle').show();
					var BergwertungScore=Math.ceil(getSlopeProportion());
					var imgurl='';
					switch(true){
...
function getSlopeProportion(){
	...
	if(heightLength>0){
		...
		chart.redraw();
		$('#container2').show();
		return getBergwertung(maxElevation,height[0],totalDistance,sqrfeed,upDistance);
	}
}

//Bergwertung=((平均坡度(%))^(3/2*坡度比例)*總爬升量(公尺))/100
function getBergwertung(maxHeight,startHeight,totalDistance,sqrfeed,upDistance){
